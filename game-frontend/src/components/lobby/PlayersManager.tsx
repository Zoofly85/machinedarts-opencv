...
export default function PlayersManager() {
  ...
  const fetchProfiles = useCallback(async () => {
    ...
    try {
      ...
      const list: PlayerProfile[] = Array.isArray(data.players) ? data.players : [];
      setProfiles(list);
    } catch (err) {
  ...
  const fetchPlayerBots = useCallback(async () => {
    try {
      ...
      const data = await response.json();
      const playerList: PlayerProfile[] = Array.isArray(data.players) ? data.players : [];
      
      // Fetch bot status for each player
      const botStatuses = await Promise.all(
        playerList.map(async (player) => {
          try {
            const statusResponse = await fetch(`${API_URL}/api/players/${player.id}/bot-status`);
            if (statusResponse.ok) {
              const status: PlayerBotStatus = await statusResponse.json();
              return status;
            }
          } catch (error) {
                    ...
                    >
                      ...
                      {playerBots
                        ...
                        .map((bot) => (
                          ...
                          >
                            {bot.isUnlocked ? '🎯' : '🔒'} {bot.playerName} - {bot.completedLegs}/50 ({Math.round(bot.progressPercentage)}%)
                          </option>
                        ))}
                      {playerBots.length === 0 && (
                      ...
                      <div className="text-xs text-zinc-400">
                        {botStats.get(player.botLevel)!.ppr !== null ? (
                          <span className="text-emerald-400 font-semibold">
                            PPR: {botStats.get(player.botLevel)!.ppr?.toFixed(2)}
                          </span>
                        ) : (
