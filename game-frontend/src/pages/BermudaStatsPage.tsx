...
interface StatsLocationState {
  summary?: BermudaSummaryState;
  players?: PlayerConfig[];
}

export default function BermudaStatsPage() {
  ...
  return (
    <div className="min-h-screen w-full bg-black text-white relative overflow-hidden flex flex-col">
      ...
      <header className="relative z-10 w-full px-6 py-6 border-b border-white/10 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-extrabold">Bermuda Triangle Stats</h1>
          <p className="text-sm text-zinc-400">13 rounds • zero-round halves your score</p>
        </div>
      ...
      <main className="relative z-10 flex-1 px-4 md:px-10 pb-10">
        <div className="max-w-6xl mx-auto mt-6 space-y-6">
              ...
              <tbody>
                {players.map((player, idx) => (
                  <tr key={player.name + idx} className="border-t border-white/5 odd:bg-black/30 even:bg-black/40">
                    ...
                    <td className="px-4 py-3 text-right text-sm text-zinc-300">{player.legsWon ?? 0}</td>
                    <td className="px-4 py-3 text-right text-sm text-zinc-300">{player.setsWon ?? 0}</td>
          ...
          {summary.turnHistory && summary.turnHistory.length > 0 && (
            <div className="rounded-2xl border border-white/10 bg-black/40 px-6 py-5">
              <h2 className="text-lg font-semibold mb-3">Recent Rounds</h2>
              <div className="overflow-x-auto">
                <table className="w-full min-w-[640px] border-separate [border-spacing:0]">
                  <thead>
                    <tr className="bg-black/60">
                      <th className="px-3 py-2 text-left text-xs uppercase tracking-[0.3em] text-zinc-500">Round</th>
                      <th className="px-3 py-2 text-left text-xs uppercase tracking-[0.3em] text-zinc-500">Player</th>
                      <th className="px-3 py-2 text-left text-xs uppercase tracking-[0.3em] text-zinc-500">Target</th>
                      <th className="px-3 py-2 text-right text-xs uppercase tracking-[0.3em] text-zinc-500">Score</th>
                    </tr>
                  </thead>
                  <tbody>
                    {summary.turnHistory.slice(-20).map((turn, idx) => (
                      <tr key={idx} className="border-t border-white/5">
                        <td className="px-3 py-2 text-sm text-zinc-400">{turn.roundIndex + 1}</td>
                        <td className="px-3 py-2 text-sm text-white">{players[turn.playerIndex]?.name || `P${turn.playerIndex + 1}`}</td>
                        <td className="px-3 py-2 text-sm text-zinc-300">{turn.target}</td>
                        <td className="px-3 py-2 text-sm text-emerald-300 text-right">{turn.roundScore}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
