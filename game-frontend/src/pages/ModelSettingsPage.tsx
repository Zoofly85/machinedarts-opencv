...
export default function ModelSettingsPage() {
  ...
  const [diffThreshold, setDiffThreshold] = useState<number>(0.25);
  ...
  const [cpuTuningEnabled, setCpuTuningEnabled] = useState<boolean>(false);
  const [cpuTuningLoading, setCpuTuningLoading] = useState<boolean>(false);
  const [cpuTuningSaving, setCpuTuningSaving] = useState<boolean>(false);
  const [cpuTuningMessage, setCpuTuningMessage] = useState<string | null>(null);
  const [cpuTuningError, setCpuTuningError] = useState<string | null>(null);
  ...
  const loadCpuTuning = async () => {
    setCpuTuningLoading(true);
    setCpuTuningError(null);
    try {
      const resp = await fetch(`${API_BASE}/api/system/cpu-tuning`);
      const contentType = resp.headers.get("content-type") || "";
      const payload = contentType.includes("application/json")
        ? await resp.json()
        : await resp.text();
      if (!resp.ok) {
        const message =
          typeof payload === "string"
            ? payload
            : payload?.detail || payload?.message || `Failed to load CPU tuning (${resp.status})`;
        throw new Error(message);
      }
      if (typeof payload?.enabled === "boolean") {
        setCpuTuningEnabled(payload.enabled);
      }
    } catch (err: any) {
      setCpuTuningError(err?.message || "Unable to load CPU tuning setting.");
    } finally {
      setCpuTuningLoading(false);
    }
  };

  const saveCpuTuning = async () => {
    setCpuTuningSaving(true);
    setCpuTuningMessage(null);
    setCpuTuningError(null);
    try {
      const resp = await fetch(`${API_BASE}/api/system/cpu-tuning`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ enabled: cpuTuningEnabled }),
      });
      const contentType = resp.headers.get("content-type") || "";
      const payload = contentType.includes("application/json")
        ? await resp.json()
      ...
      if (!resp.ok) {
        const message =
          typeof payload === "string"
            ? payload
            : payload?.detail || payload?.message || "Failed to update CPU tuning.";
        throw new Error(message);
      }
      setCpuTuningMessage("CPU tuning saved. Restart backend to apply.");
    } catch (err: any) {
      setCpuTuningError(err?.message || "Failed to update CPU tuning.");
    } finally {
      setCpuTuningSaving(false);
    }
  };
      ...
      <main className="relative z-10 w-full px-4 sm:px-6 md:px-10 pb-10">
        ...
        <div className="mt-6 rounded-xl border border-white/10 bg-white/5 p-4">
          <div className="flex items-center justify-between mb-2">
            <h2 className="text-lg font-semibold">CPU tuning</h2>
            <button
              type="button"
              onClick={loadCpuTuning}
              disabled={cpuTuningLoading}
              className="text-xs px-3 py-2 rounded-lg border border-zinc-700 bg-zinc-800 hover:bg-zinc-700 transition-colors disabled:opacity-60"
            >
              Refresh
            </button>
          ...
          <p className="text-xs text-zinc-400 mb-4">
            Auto-tunes OpenVINO and OMP thread counts. Enable for maximum throughput.
            Disable if detection becomes unstable on some PCs.
          </p>
          <label className="flex items-center gap-3 text-sm font-semibold">
            <input
              type="checkbox"
              checked={cpuTuningEnabled}
              onChange={(event) => setCpuTuningEnabled(event.target.checked)}
          ...
          <div className="mt-4 flex items-center gap-3">
            <button
              type="button"
              onClick={saveCpuTuning}
              disabled={cpuTuningSaving || cpuTuningLoading}
              className="px-4 py-2 rounded-lg bg-red-600 hover:bg-red-700 text-white text-sm font-semibold disabled:opacity-60 disabled:cursor-not-allowed"
            >
              {cpuTuningSaving ? "Saving..." : "Save (restart required)"}
            </button>
            <span className="text-xs text-zinc-400">
              Takes effect after restarting the backend.
            </span>
          </div>
        </div>
