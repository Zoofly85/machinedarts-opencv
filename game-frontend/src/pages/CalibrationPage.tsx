...
import { ArrowLeft, Camera, RotateCw, RotateCcw, Save, Crosshair, CheckCircle, Focus, X, AlertCircle } from "lucide-react";
...
export default function CalibrationPage({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  const isDeviceAlreadyUsed = (deviceIndex: number, slot: number) =>
    slotSelections.some((idx, idxSlot) => idxSlot !== slot && idx === deviceIndex);
  ...
  const fetchCameraDevices = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/camera/devices?max_devices=10`);
      const data = await response.json();
  ...
  const handleCameraSlotChange = (slot: number, value: number | null) => {
  ...
  const saveCameraSelection = async () => {
    ...
    try {
      ...
      setSelectionMessage({
        type: "success",
        text: "Camera selection saved. Re-run calibration for any slots that changed.",
      });
      await fetchCameras();
  ...
  const fetchCalibrationStatus = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/calibration/status/${selectedCamera}`);
      const data = await response.json();
  ...
  const connectToWebSocket = () => {
    ...
    const ws = new WebSocket(`ws://localhost:8000/ws/camera/${selectedCamera}`);
  ...
  const connectToStatusWebSocket = () => {
    ...
    // Create new WebSocket connection
    const ws = new WebSocket(`ws://localhost:8000/ws/calibration/status`);
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (data.calibration_statuses) {
          const status = data.calibration_statuses.find(
            (s: CalibrationStatus) => s.camera_index === selectedCamera
          );
          if (status) {
  ...
  const runDetection = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/calibration/detect/${selectedCamera}?include_inner_points=${includeInnerPoints ? "true" : "false"}`, {
        method: 'POST'
      });
  ...
  const startLiveFocusMonitoring = async (centerX: number, centerY: number) => {
    ...
    focusIntervalRef.current = setInterval(fetchLiveFocus, 500);
  };
  ...
  return (
    <div className={`${embedded ? "min-h-full" : "h-dvh"} w-full bg-black text-white relative overflow-hidden flex flex-col`}>
      ...
      {!embedded && (
        <header className="relative z-10 w-full px-4 sm:px-6 md:px-10 py-3 sm:py-4 flex items-center justify-between flex-shrink-0">
        <Logo />
        <ActionButton
          href="/"
          label="Back"
      ...
      <main className={`relative z-10 w-full px-4 sm:px-6 md:px-10 flex-1 flex flex-col overflow-hidden min-h-0 ${embedded ? "pt-4" : ""}` }>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] xl:grid-cols-[1fr_380px] gap-3 sm:gap-4 lg:gap-6 flex-1 min-h-0">
          ...
          >
            ...
            <div
              className={`relative w-full bg-black rounded-lg overflow-hidden mb-2 flex-1 min-h-0 ${
            ...
            <section className="mb-3 sm:mb-4 rounded-xl border border-zinc-800 bg-zinc-900/40 p-3 sm:p-4">
              <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                ...
                <button
                  ...
                  className="text-xs px-3 py-2 rounded-lg border border-zinc-700 bg-zinc-800 hover:bg-zinc-700 transition-colors"
                >
                  Refresh Devices
                </button>
              </div>
                ...
                <div className="mt-3 grid gap-3 sm:grid-cols-3">
                  {cameras.map((camera, slot) => {
                    ...
                    return (
                      <div
                        key={camera.index}
                        className="bg-black/30 border border-zinc-800 rounded-lg p-3 flex flex-col gap-2"
                      >
                        <div className="flex items-center justify-between text-sm font-semibold">
                          <span>{camera.name}</span>
                          <span className="text-xs text-zinc-400">Slot #{camera.index}</span>
                        </div>
                        <select
                          ...
                          onChange={(e) => {
                            ...
                            handleCameraSlotChange(slot, value === "" ? null : Number(value));
                          }}
                        ...
                        >
                          ...
                          {cameraDevices.map((device) => (
                            <option
                              key={device.index}
                              value={device.index}
                              disabled={isDeviceAlreadyUsed(device.index, slot)}
                            >
                              {`${device.label || `Device ${device.index}`} ${
                                device.available
                                  ? ""
                                  : device.status === "busy"
                                  ? "(busy)"
              ...
              <div className="mt-3 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div className="text-xs text-yellow-300 flex items-center gap-2">
                  ...
                  Changing camera hardware requires recalibrating that slot to maintain accuracy.
                </div>
                <button
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-1.5 sm:gap-2 flex-shrink-0">
              ...
              <button
                ...
                disabled={isLoading || !calibrationStatus?.is_calibrated}
