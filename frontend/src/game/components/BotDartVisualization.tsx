...
const BotDartVisualization: React.FC<BotDartVisualizationProps> = ({
  ...
  // Dartboard dimensions in SVG coordinates, scaled from the calibration
  // constants where the double outer edge is the real 170 mm board radius.
  const R = {
    outer: 100,
    doubleOuter: 85,
    doubleInner: 80,
  ...
  // Convert cm to SVG units (double outer = 85, real board radius = 17 cm)
  const CM_TO_SVG = R.doubleOuter / 17.0;

  const getNumber = (x: number, y: number): [number, string] => {
    const angle = Math.atan2(y, x);
    const relativeAngle = (angle - boardStart + 2 * Math.PI) % (2 * Math.PI);
    const num = order[Math.floor(relativeAngle / seg)];
    const radius = Math.sqrt(x * x + y * y);

    if (radius <= R.bullInner) return [25, 'D'];
    if (radius <= R.bullOuter) return [25, 'S'];
    if (radius > R.doubleOuter) return [0, 'S'];
  ...
  const generateDartPositions = () => {
    ...
    if (targetNumber === 25) {
      ...
      const index = order.indexOf(targetNumber);
      targetAngleCm = boardStart + index * seg + seg / 2;
