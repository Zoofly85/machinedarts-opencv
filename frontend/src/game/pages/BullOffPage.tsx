...
export default function BullOffPage() {
    ...
    (startingPlayer: number) => {
      ...
      switch (state.selectedGame) {
        ...
        case "one_two_one":
          ...
          break;
        case "pacman":
          navigate("/pacman", { state: updatedState });
          break;
        default:
          navigate("/game");
      }
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeDetection(async (payload) => {
      ...
      if (payload.event === "dart_score_unavailable") {
        setMessage("Score unavailable for that dart. Remove it and throw again.");
        return;
      }

      if (payload.event !== "dart_score") {
        return;
      }

      try {
        const response = await fetch(`${API_URL}/api/detection/scores?raw=true`);
        ...
        const hasDistance = typeof position?.distance_mm === "number" && Number.isFinite(position.distance_mm);
        if (!score || !hasDistance) {
          setMessage("Bull-off needs a calibrated board position. Remove the dart and try again.");
          return;
        }
        const result: BullOffResult = {
          playerIndex: playerIndex(currentPlayer, players),
          playerName: currentPlayer.name || `Player ${playerIndex(currentPlayer, players) + 1}`,
          distanceMm: position.distance_mm,
        ...
        <div className="h-full w-full max-w-6xl mx-auto grid grid-cols-1 xl:grid-cols-[1fr_1.1fr] gap-6">
          <div className="bg-zinc-900/60 border border-white/10 rounded-2xl p-6 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              ...
              {winnerIndex !== null && (
                <div className="rounded-xl border border-red-500/50 bg-red-500/10 px-4 py-2 text-right">
                  <p className="text-xs uppercase tracking-[0.2em] text-red-200">Auto starting</p>
                  <p className="text-sm font-semibold text-white">
                    {autoStartCountdown ?? 3}s
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="rounded-xl bg-black/50 border border-white/10 p-6 flex flex-col gap-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="rounded-xl bg-zinc-900/70 border border-white/10 p-4">
                  ...
                  <p className="text-sm text-zinc-400">
                    {winnerIndex !== null ? "Match starts automatically" : awaitingThrow ? "Waiting for hit" : "Auto-advancing"}
                  </p>
                </div>
              </div>
            </div>
