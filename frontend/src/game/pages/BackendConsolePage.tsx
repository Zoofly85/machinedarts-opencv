...
const LOG_POLL_VISIBLE_MS = 700;
const LOG_POLL_HIDDEN_MS = 5000;
const LOG_LINES = 220;
...
export default function BackendConsolePage() {
  ...
  const refreshStatus = useCallback(async () => {
    try {
      const [statusResponse, scoresResponse] = await Promise.all([
        ...
        fetch(`${API_BASE_URL}/api/detection/scores?raw=true`),
      ]);
  ...
  useEffect(() => {
    ...
    const scheduleLogs = () => {
      ...
      const delay = document.visibilityState === "visible" ? LOG_POLL_VISIBLE_MS : LOG_POLL_HIDDEN_MS;
  ...
  }, [refreshAll, refreshLogs, refreshStatus]);
