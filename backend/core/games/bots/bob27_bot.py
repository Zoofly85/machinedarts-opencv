...
class Bob27Bot:
    """Bot that always aims at the double of the current target."""

    def __init__(self, level: int):
        self.level = max(1, min(level, 9))

    def throw(self, game_state: Dict, _player_index: int) -> BotThrow:
        target = game_state.get("currentTarget") or 20
        sigma = X01_SIGMA.get(self.level, 2.0)
        number, field = _target_coordinates((Field.DOUBLE, target), sigma)
